...
#define PACK(size, alloc) ((size) | (alloc)) // size와 alloc의 값을 하나의 word로 묶는다.
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    {                                                                    // 기존 블록의 다음 블록이 가용 블록이고, 추가 사이즈가 충분하면
        ...
        {                                         // 가용 블록이고 사이즈 충분
            remove_block(NEXT_BLKP(oldptr)); // 다음 블록 제거
...
static void insert_block(void *bp, size_t size)
{
    ...
    if (search_ptr != NULL)//search_ptr이 NULL이 아닐 때, 나보다 큰 친구가 있을 때
    { 
